...
const Navbar = () => {
  ...
  const { menuOpen, user, news } = state;
  ...
  return (
    <header className={`${!visible && 'sm:-translate-y-[200%]'} fixed max-sm:bottom-0 left-0 flex-row 
    sm:top-0 sm:flex-col w-screen z-[100] transition-all duration-200`} >
        <section className='bg-[var(--primary)] h-[var(--newsHeight)] overflow-hidden z-10 text-[var(--bg)] w-screen centerAll relative'>
        ...
        <nav>
          ...
          <div onClick={()=>  setState({...state, menuOpen: !menuOpen})} className='max-sm:order-1 svgContainer cursor-pointer z-10'>
            <svg fill={'none'} viewBox="0 0 24 24" strokeWidth={1.8} stroke={'var(--text)'} className="w-8 h-8"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" /></svg>
          </div>
          {/* LOGO */}
          ...
          <div className='max-sm:order-5'>
            {user.username ? 
            <Link to={'/account'} className="svgContainer account rounded-full h-8 w-8 centerAll" 
            style={{backgroundColor: (location.pathname==='/account') ?'var(--text)':'var(--primary)'}}>
              <h4 className='uppercase text-[var(--bg)] mt-[2px]'>{user.username.slice(0,1)}</h4>
            </Link>
